...
const estadoValido    = (estado) => ESTADOS_VALIDOS.some(e => normalizar(e) === normalizar(estado));
const normalizarEstado = (estado) => ESTADOS_VALIDOS.find(e => normalizar(e) === normalizar(estado));

router.get('/', (req, res) => {
    const { estado } = req.query;
    let sql = 'SELECT * FROM ordenes', params = [];
    if (estado) { sql += ' WHERE estado = ?'; params = [normalizarEstado(estado) ?? estado]; }
...
router.get('/:id', (req, res) => {
    ...
    db.get('SELECT * FROM ordenes WHERE id = ?', [id], (err, orden) => {
        ...
        db.all(
            `SELECT p.id, p.nombre, p.precio, op.cantidad FROM orden_platos op JOIN platos p ON p.id = op.plato_id WHERE op.orden_id = ?`,
            [id], (err, platos) => {
                if (err) return res.status(500).json({ success: false, message: err.message });
...
router.post('/', (req, res) => {
    ...
    db.get('SELECT id FROM mesas WHERE id = ?', [mesaId], (err, mesa) => {
        ...
        db.run('INSERT INTO ordenes (mesaId, total, estado) VALUES (?, ?, ?)', [mesaId, Number(total), estadoFinal],
            function(err) {
                if (err) return res.status(500).json({ success: false, message: err.message });
                const ordenId = this.lastID;
                const stmt = db.prepare('INSERT INTO orden_platos (orden_id, plato_id, cantidad) VALUES (?, ?, ?)');
                for (const item of platos) stmt.run([ordenId, item.plato_id, item.cantidad]);
                stmt.finalize();
...
router.put('/:id', (req, res) => {
    ...
    db.get('SELECT * FROM ordenes WHERE id = ?', [id], (err, orden) => {
        ...
        if (!estadoValido(estado))
            return res.status(400).json({ success: false, message: "estado debe ser: En preparacion, Entregada o Cancelada" });
        const estadoFinal = normalizarEstado(estado);
        db.run('UPDATE ordenes SET mesaId = ?, total = ?, estado = ? WHERE id = ?', [mesaId, Number(total), estadoFinal, id],
            function(err) {
                if (err) return res.status(500).json({ success: false, message: err.message });
